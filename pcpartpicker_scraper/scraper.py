        ...
        try:
            ...
            if part == "memory":
                ...
                for ecc_type, url in urls.items():
                    ...
                    manufacturers, product_set = self.get_part_data_for_url(
                        url)
                    # This will merge the manufacturer list from the per-ECC query back to the final
                    # list of manufacturers
                    total_manufacturers_set = total_manufacturers_set | set(
                        manufacturers)
                    ...
                    product_set = set(
                        map(lambda t: t[:-1] + (ecc_type,) + t[-1:], product_set))
...
def get_manufacturers(driver: uc.Chrome) -> List[str]:
    html = lxml.html.fromstring(driver.page_source)
    try:
        show_more = WebDriverWait(driver, 15).until(
            lambda x: x.find_element(By.XPATH, '//*[@id="m_set"]/a[1]'))
        center_element(driver, show_more)
        click(driver, show_more)
    except TimeoutException:
        pass
    manufacturers = html.xpath(
        '//*[@id="m_set"]/li[contains(@id, "li_")]/label/text()')
