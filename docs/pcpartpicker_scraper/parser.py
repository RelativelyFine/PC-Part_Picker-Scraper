...
from .mappings import currency_classes, currency_symbols, part_classes, none_symbols
...
class Parser:
    ...
    def parse(self, parts: List[List[str]]):
        ...
        part_list.sort(key=lambda x: (
            x.brand, x.model if isinstance(x.model, str) else ""))
        return part_list
    ...
    def parse_token(self, data: list) -> object:
        ...
        try:
            ...
            logger.error(
                f"{parsed_data} is not valid input data for {_class}!")
    ...
    def retrieve_brand_info(self, brand_information: str) -> Tuple[Optional[str], Optional[str]]:
        for x in range(len(brand_information) + 1):
            ...
            return brand_information, brand_information
